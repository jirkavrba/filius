...
public class DHCPServer extends UDPServerAnwendung {
     ...
     * Zunaechst wird dazu die Liste vergebener IP-Adressen aufgeraeumt (d.h. abgelaufene Eintraege entfernt) mit Aufruf
     * von cleanUp()
     */
    ...
    /**
     * Ein eventuell vorhandener Eintrag zur uebergebenen MAC-Adresse wird geloescht.
     */
     ...
     * <ol>
     * <li>Beginnend mit der Untergrenze werden alle IP-Adressen im Intervall von Untergrenze bis Obergrenze geprueft,
     * ob sie noch zu vergeben sind.</li>
     * <li>die gefundene IP-Adresse wird zunaechst mit einer kurzen TTL reserviert, d.h. in die Liste vergebener
     * IP-Adressen eingetragen (TTL = 4 sek. mal Verzoegerungsfaktor der Verbindungsleitungen)</li>
     ...
     * 
     */
    synchronized String reserviereFreieIP(String maddr, String requestedIpAddress) {
        Main.debug.println("INVOKED (" + this.hashCode() + ", T" + this.getId() + ") " + getClass()
                + " (DHCPServer), reserviereFreiIP(" + maddr + ")");
        String freieIP;

        synchronized (ipListe) {
            if (inRange(requestedIpAddress) && istIPAdresseFrei(requestedIpAddress)) {
                freieIP = requestedIpAddress;
            } else {
    ...
    /**
     * Weisst einer mac-Adresse eine IP für die angegebene ttl zu. Diese Funktion wird normalerweise nur von den
     * DHCPServerMitarbeitern ausgeführt, kann aber auch "manuell" aufgerufen werden. Das richtige Setzen der ttl ist
     * hierbei zu beachten. Sobald System.currentMills() >= ttl ist, ist die time to life abgelaufen. die ttl muss also
     * entsprechend hoch gesetzt werden. <br />
     * <b>Achtung! </b> Wenn die angeforderte Adresse bereits vergeben ist, erfolgt keine Zuweisung!
     * 
     * @param mac
     * @param ip
     * @param ttl
    ...
    /**
     * Berechnet die folgende IP-Adresse an Hand der Netzmaske. Wenn die Obergrenze erreicht ist, wird <b>nicht</b>
     * wieder bei der Untergrenze begonnen. <br />
     * Dazu wird zunaechst geprueft, ob die Netzkennung der uebergebenen IP-Adresse identisch zu denen der Unter- und
     * Obergrenze ist. <br />
     * Dann wird geprueft, ob die Rechnerkennung der uebergebenen IP-Adresse zwischen Unter- und Obergrenze liegt. <br />
     * Schliesslich wird die naechste Rechnerkennung zurueck gegeben.
     * 
     * @param ip
     *            Die IP-Adresse, zu der die folgende Adresse gesucht wird.
     * @return die Folgeadresse oder null, wenn es keine Adresse im Intervall zwischen Unter- und Obergrenze gibt, die
     *         auf die uebergebene Adresse folgt.
     */
    private String naechsteIPAdresse(String ip) {
        ...
        // jeweils als 3-Tupel von Zahlen
