...
/**
 * <p>
 * Der DHCPServerMitarbeiter hat eine andere Funktion als dies bei sonstigen
 * ServerMitarbeiter-Objekten der Fall ist. Dieser Mitarbeiter verarbeitet alle
 * eingehenden UDP-Segmente, weil alle DHCP-Clients mit der Absender-Adresse
 * 0.0.0.0 und dem UDP-Port 68 Nachrichten verschicken. Daher funktioniert das
 * Demultiplexen auf der Transportschicht nicht.
 * </p>
 * 
 * <p>
 * Dieser Mitarbeiter verwaltet daher fuer jede MAC-Adresse, zu der eine
 * Verbindung besteht, einen eigenen Zustand in einer Hash-Tabelle, der ueber
 * das folgende Verhalten entscheidet. Der Socket wird nicht geschlossen, weil
 * immer auf weitere eingehende Verbindungsanfragen gewartet wird.
 * </p>
 * 
 * Die verwendeten DHCP-Nachrichten sind stark vereinfacht, weil sie beobachtet
 * werden können. Der Aufbau einer DHCP-Nachricht besteht aus folgenden
 * Komponenten:
 * <ul>
 ...
 * <ul>
 * <li>DHCPDISCOVER: Ein Client ohne IP-Adresse sendet eine Broadcast-Anfrage
 * nach Adress-Angeboten an den/die DHCP-Server im lokalen Netz.</li>
 * <li>DHCPOFFER: Der/die DHCP-Server antworten mit entsprechenden Werten auf
 * eine DHCPDISCOVER-Anfrage.</li>
 * <li>DHCPREQUEST: Der Client fordert (eine der angebotenen) IP-Adresse(n),
 * weitere Daten sowie Verlaengerung der Lease-Zeit von einem der antwortenden
 * DHCP-Server.</li>
 ...
 * verwendet wird.
 * <li>
 * <li>DHCPRELEASE: Der Client gibt die eigene Konfiguration frei, damit die
 * Parameter wieder für andere Clients zur Verfuegung stehen.</li>
 * <li>DHCPINFORM: Anfrage eines Clients nach Daten ohne IP-Adresse, z. B. weil
 * der Client eine statische IP-Adresse besitzt.</li>
 * </ul>
...
public class DHCPServerMitarbeiter extends ServerMitarbeiter {
	 ...
	 * machen Vorschlaege für eine IP-Adresse. Dies geschieht ebenfalls mit
	 * einem Broadcast an die Adresse 255.255.255.255 mit UDP-Quellport 67 und
	 * UDP-Zielport 68.
	 * </p>
	 * 
	 * <p>
	 ...
	 * anderer Rechner die Adresse verwendet. Dies geschieht ueblicherweise
	 * durch einen ARP-Request mit der soeben zugeteilten IP-Adresse. Antwortet
	 * ein anderer Host im Netz auf diesen Request, so wird der Client die
